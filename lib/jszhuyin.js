  ...
  var IMEngineDatabase = function imedb() {
    ...
    var indexJSON = function imedb_indexJSON(s, response) {
        for (var e in response[s]) {
          ...
          for (var key in keys) {
            key = keys[key];
            if (!(key in jsonDataIndex)) jsonDataIndex[key] = [];
            jsonDataIndex[key].push(jsonDataStore.length - 1);
          }
        }
    };
    ...
    */
    ...
    this.searchInIndex = function imedb_searchInIndex(syllablesStr, indexOnly) {
      ...
      if (indexOnly) return jsonDataIndex[syllablesStr];
      for(var index in jsonDataIndex[syllablesStr]) {
        found.push(jsonDataStore[jsonDataIndex[syllablesStr][index]]);
      }
      //debug(found);
      return found;
    };
    ...
    this.multipleIndexSearch = function imedb_multipleIndexSearch(syllables) {
        ...
        var map = {};
        syllables.forEach(function (element, index) {
            element = element.replace('*', '')
            self.searchInIndex(element, true).forEach(function (element, index) {
                if (!(element in map)) map[element] = 0;
                map[element] += 1; 
            });
        });
        var found = [];
        for(var ele in map) {
            if (map[ele] == syllables.length)
                found.push(jsonDataStore[ele]);
        }
        debug(found);
        return found;
    };
