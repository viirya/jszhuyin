...
(function() {
      ...
      lookup(syllablesForQuery, 'term', function lookupCallback(terms) {
        ...
        if (syllablesInBuffer.length === 1) {
          ...
          if (!candidates.length) {
            ...
            var syllable = syllablesInBuffer.join('').replace(/\*/g, '');
  ...
  var IMEngineDatabase = function imedb() {
    ...
    var genIndexKey = function imedb_genIndexKey(words, word_index) {
        //debug('genIndexKey');

        if (word_index == words.length) return [];

        var word = words[word_index];
        var elements = word.split('');
        var keys = [];
        var keys_of_nextlevel = genIndexKey(words, word_index + 1);

        for (var index in elements) {
            var item = elements.slice(0, index);
            ...
            if (keys_of_nextlevel.length == 0) {
                keys.push(item.join(''));
                continue;
            }
            
            for (var key in keys_of_nextlevel) {
                key = keys_of_nextlevel[key];
                keys.push(item.join('') + key);
            }
        }
        return keys;
    };

    var indexJSON = function imedb_indexJSON(s, response) {
        for (var e in response[s]) {
          jsonDataStore.push(response[s][e]);
        
          // for exact matching
          if (!(s in jsonDataIndex)) jsonDataIndex[s] = [];
          jsonDataIndex[s].push(jsonDataStore.length - 1);
        
          // creating index for incompleteMatching
          var words = s.split('-');
          var keys = genIndexKey(words, 0);
          ...
          for (var key in keys) {
            ...
            var root = jsonDataIndex2;
            for(var ele_index in elements) {
                var element = elements[ele_index];
                
                if (!(element in root)) root[element] = {};

                if ((parseInt(ele_index) + 1) < elements.length) {
                    if (!('next' in root[element]))
                        root[element]['next'] = {};
                    root = root[element]['next'];
                } else {
                    if (!('terms' in root[element]))
                        root[element]['terms'] = {};
                    root[element]['terms'][jsonDataStore.length - 1] = 1;
                }
            }
          }
        }
    ...
    */
      ...
      getTermsInDB(function getTermsInDBCallback() {
        ...
        req.onsuccess = function getdbSuccess(ev) {
          ...
          getTermsJSON(function getTermsInDBCallback() {
            if (!jsonDataIndex && !jsonDataStore) {
    ...
    this.searchInIndex = function imedb_searchInIndex(syllablesStr, indexOnly) {
      var found = [];
      //debug(jsonDataIndex[syllablesStr]);

      var syllables = syllablesStr.split('');
      var root = jsonDataIndex2;
      var indexes;
      for(var syllable_index in syllables) {
        var syllable = syllables[syllable_index];
        if (!(syllable in root))
            break;
        
        if ('terms' in root[syllable])
            indexes = root[syllable]['terms'];
        else
            indexes = [];

        if ('next' in root[syllable])
            root = root[syllable]['next'];
        else
            break;
      }

      if (indexOnly) return indexes;
      for(var index in indexes) {
        found.push(jsonDataStore[index]);
      }
      //debug(found);
      return found;
    };

    this.multipleIndexSearch = function imedb_multipleIndexSearch(syllables) {
        debug('multipleIndexSearch');
        syllables = syllables.join('');
        syllables = syllables.replace(/\*/g, '');
        return self.searchInIndex(syllables);
    };
    ...
    this.getTerms = function imedb_getTerms(syllables, callback) {
      ...
      if (jsonData) {
        debug('Lookup in JSON.');
        if (!matchRegEx) {
          callback(self.searchInIndex(syllablesStr, false) || false);
          return;
        }
        debug('Do range search in JSON data.');
        var result = [];
        var dash = /\-/g;
        result = self.multipleIndexSearch(syllables);
